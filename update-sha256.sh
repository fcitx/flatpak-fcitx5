...
BAZEL_VER=$(flatpak info org.freedesktop.Sdk.Extension.bazel|awk '{if ($1=="Version:") print $2}')
if [[ "$BAZEL_VER" > "7.1.2" ]];then
	# --enable_bzlmod
	bazel_flags="--config bzlmod"

	# --noenable_bzlmod
	#bazel_flags="--config workspace"
else
	bazel_flags="--config workspace"
	bazel_flags="--config bzlmod"
fi
...
BUILD_TARGET="unix/fcitx5:fcitx5-mozc.so server:mozc_server gui/tool:mozc_tool package"
bazel clean --expunge
bazel build --nobuild --experimental_repository_resolved_file=resolved.bzl --repository_cache="$_MOZC_BAZEL_CACHE" --config oss_linux --config release_build $BUILD_TARGET $bazel_flags
...
do
       ...
       if [[ -z $sha ]];then
	       wget -nc $url
	       sha=$(sha256sum $filename|cut -f1 -d" ")
       fi
...
jq -r -f $BASE/resolved.jq $BASE/resolved.json| yq -y > $BASE/mozc-deps.yaml
popd
